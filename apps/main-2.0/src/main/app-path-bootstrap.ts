...
export function bootstrapApplicationPaths(options: BootstrapApplicationPathsOptions): ApplicationPaths {
  ...
  const explicitHome = absolutePath(env.AGENT_RECALL_HOME_DIR);
  const explicitAppData = absolutePath(env.AGENT_RECALL_APP_DATA_DIR);
  const explicitUserData = absolutePath(env.AGENT_RECALL_USER_DATA_DIR);
  ...
  const appData = registerApplicationPath(
    ...
    "appData",
    explicitAppData
      ?? (explicitHome ? platformAppDataPath(platform, env, home) : electronPath(options.app, "appData", warn))
